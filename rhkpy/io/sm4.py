"""Reader for RHK SM4 files (RHK Technology Inc.) and conversion to xarray.

The binary reader (enums, RHKsm4 and the RHK* object classes) is ported
verbatim from the spym project by Mirco Panighel (https://github.com/rescipy-project/spym,
MIT license), which rhkpy previously vendored. The vendored copy is kept in
the repository as a legacy reference implementation, used only by the test
suite to cross-validate this module.

Public entry points:

- :func:`load` - low-level: returns the RHKsm4 page container
- :func:`to_dataset` - returns an xarray Dataset (one DataArray per page)
- :func:`load_dataset` - like to_dataset, with the `filename` attribute set
  and load errors returned as None (this is what :class:`rhkpy.rhkdata` uses)
"""
...
## loading and conversion to xarray -----------------------------------
## (adapted from spym.io.rhksm4._methods and spym.io.load)
...
def load_dataset(filename, scaling=True):
    ...
    if not filename.lower().endswith('.sm4'):
        raise ValueError(f'Not an .sm4 file: {filename}')

    try:
        ds = to_dataset(filename, scaling=scaling)
    except Exception:
